...
// Two official team names are noticeably longer than the rest of the grid
// and force aggressive truncation in tight columns — shorten just those two
// rather than shrinking every name's font to accommodate the longest case.
const TEAM_SHORT_NAME = {
  'Red Bull Racing': 'Red Bull',
  'Haas F1 Team': 'Haas',
};
function shortTeam(name) {
  return TEAM_SHORT_NAME[name] || name;
}
...
function renderLastRaceResults(lastRace) {
  ...
  const row = (r) => `
    <div class="result-row ${r.position === 1 ? 'p1' : ''}">
      ...
      <span class="rteam">${shortTeam(r.team)}</span>
  ...
  const flBanner = fl ? `
    <div class="fastest-lap-banner">
      <svg class="icon-fl" viewBox="0 0 24 24"><path d="M13 2 3 14h7l-1 8 10-12h-7l1-8z"/></svg> Fastest Lap: <span class="fl-name">${fl.name}</span> <span class="fl-time">${fl.durationFormatted}</span>
    </div>` : '';
  ...
  el.innerHTML = `
    ...
    <div class="result-banners">
      <div class="winner-banner" style="border-color:${lastRace.winning_team_colour}">
        <svg class="icon-trophy" viewBox="0 0 24 24"><path d="M6 3h12v2h3v3a4 4 0 0 1-4 4h-.1a6 6 0 0 1-3.9 3.86V18h3v2H8v-2h3v-2.14A6 6 0 0 1 7.1 12H7a4 4 0 0 1-4-4V5h3V3zm0 4H5v1a2 2 0 0 0 2 2V7zm12 0v3a2 2 0 0 0 2-2V7h-2z"/></svg> <span class="wb-team">${shortTeam(lastRace.winning_team)}</span> wins
      </div>
      ${flBanner}
    </div>
...
function renderStandings(drivers, teams, asOfRace) {
  ...
  const driverRows = (drivers || []).slice(0, 11).map((d) => `
    <div class="standing-row">
      ...
      <span class="sname">${d.code || d.name} <span class="steam">${shortTeam(d.team)}</span></span>
  ...
  const teamRows = (teams || []).slice(0, 11).map((t) => `
    <div class="standing-row">
      ...
      <span class="sname">${shortTeam(t.team)}</span>
