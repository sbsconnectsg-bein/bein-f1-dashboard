...
async function main() {
  ...
  if (lastRaceSession) {
    ...
    const [driversMeta, champDrivers, champTeams, results] = await Promise.all([
      getJSON(`${BASE_URL}/drivers?session_key=${sk}`),
      getJSON(`${BASE_URL}/championship_drivers?session_key=${sk}`),
      getJSON(`${BASE_URL}/championship_teams?session_key=${sk}`),
      getJSON(`${BASE_URL}/session_result?session_key=${sk}`),
    ]);
